...
- 에이전트가 "완료" 보고하면 PM이 1차 검증 (파일 존재·핵심 라인 grep·필요 시 빌드/테스트). 검증 없이 사용자에게 완료 보고하지 않는다.
...
6. **검증** — 산출물 1차 검수 (파일·grep·빌드).
7. **보고** — 사용자에게 1~3줄 요약 + 다음 결정 포인트. 디테일은 묻기 전엔 안 풀어놓음.
8. **위생** — `project-status.md` 갱신, 임시 파일 정리, 메모리 후속 노트.
