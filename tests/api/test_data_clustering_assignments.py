"""W10-C.4a: GET /projects/{pid}/scans/{sid}/data/clustering/assignments."""
...
@pytest.mark.asyncio
async def test_assignments_404_when_missing(
    ...
    async with await _client(pg_session, user) as client:
        r = await client.get(
            f"/api/v1/projects/{pid}/scans/{sid}/data/clustering/assignments"
        )
...
@pytest.mark.asyncio
async def test_assignments_returns_json(
    tmp_path, monkeypatch, pg_session, sample_user_factory, sample_scan_factory
):
    user = await sample_user_factory()
    scan = await sample_scan_factory()
