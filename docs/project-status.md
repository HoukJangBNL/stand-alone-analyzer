...
**Current branch**: `feat/migration-cutover` (post-cutover punch list 진행 중 — W3.5 / W2.4 / W8.1 / **W6 (auth+ACL+usage)** / **W5-A (schema)** / **W5-infra (AWS S3)** 완료. W6.1 Cognito 부트스트랩만 AWS 승인 대기. 다음은 W5-B1 → W5-B2 → W5-C, 그리고 W7 / W8.2 / W9.)
...
### 3.1 다음 한 발
**W5-A (스키마, alembic 0003) 완료(2026-05-22)** + **W5-infra (AWS S3 인프라 7 게이트 모두 적용) 완료(2026-05-22)**. 다음은 **W5-B1**(materials + scan create endpoints) → **W5-B2**(presign + complete + finalize) → **W5-C**(React 업로드 모달). 그 후 **W6.1 Cognito 부트스트랩**(RDS `0003` 마이그레이션 적용 포함). 남은 sketch는 W7(GPU) / W8 Part 2(SSM) / W9(reg_covar 캘리브레이션).
