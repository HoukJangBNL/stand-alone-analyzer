...
### 3.1 다음 한 발
**W3.5 인터랙티브 클러스터링 튜너 UI.** W4.4 backend가 끝났고 W3.5 플랜도 detailed 단계로 작성됨. api-developer가 backend slice → frontend-architect가 UI 슬라이더+auto-tune 버튼 순으로 dispatch. 플랜: [`docs/superpowers/plans/2026-05-21-W3.5-clustering-tuner.md`](superpowers/plans/2026-05-21-W3.5-clustering-tuner.md).

### 3.2 백로그 (우선순위 순)

> 모든 백로그 항목에 대해 플랜 작성 완료. 실행 가능 여부는 각 플랜의 "Execution Handoff" 섹션을 참고.

- [ ] **W3.5** 인터랙티브 클러스터링 튜너 UI — [`plan (READY)`](superpowers/plans/2026-05-21-W3.5-clustering-tuner.md) — api-developer + frontend-architect, 10 tasks (TDD red→green).
- [ ] **W2.4** manifest 엔드포인트 DB rewire — [`plan (READY)`](superpowers/plans/2026-05-21-W2.4-manifest-db-rewire.md) — db-specialist + api-developer, 6 tasks. W2.1 ProjectContext + W4.2 db.reconcile 브리지.
...
- [ ] **W6** 인증/세션 (`system` 스텁 → 실제 사용자) — [`plan (SKETCH)`](superpowers/plans/2026-05-21-W6-auth-session.md) — D1–D6 결정 필요(IdP·토큰 모양·upsert 정책·ACL·UI flow·W5 vs W6 순서).
...
- [ ] **W8 (Part 1)** CI: alembic 모델↔스키마 drift check — [`plan (READY, Part 1)`](superpowers/plans/2026-05-21-W8-ci-drift-and-ssm.md) — devops-engineer (or api-developer), 3 tasks. 새 워크플로 `.github/workflows/alembic-drift.yml`.
