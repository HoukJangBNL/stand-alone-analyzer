...
**Current branch**: `feat/migration-cutover` (post-cutover punch list 진행 중 — W3.5 / W2.4 / W8.1 / **W6 (auth+ACL+usage)** / **W5 전체(A/infra/B1/B2/C)** + **admin_usage 격리 fix** 완료. W6.1 Cognito 부트스트랩만 AWS 승인 대기. 다음은 Playwright e2e + 사용자 수동 테스트 또는 W6.1 / W7 / W8.2 / W9.)
...
### 3.1 다음 한 발
**W5 전체 완료(2026-05-22)** — W5-A 스키마 + W5-infra AWS S3 + W5-B1 materials/scan create + W5-B2 presign/complete/finalize + W5-C React 업로드 모달. **admin_usage 테스트 격리 이슈도 수정**(`21e9640`, `tests/db/conftest.py` SAVEPOINT join). 다음: **W6.1 Cognito 부트스트랩** (AWS 승인 대기, RDS `0002`/`0003` 마이그레이션 적용 포함) 또는 **Playwright e2e + 사용자 수동 테스트**. 남은 sketch는 W7(GPU) / W8 Part 2(SSM) / W9(reg_covar 캘리브레이션).
...
- [ ] **W10** real projects table + 1:N scans + per-scan analysis + per-scan mutex/manifest — D-block 잠금(2026-05-22) → 5개 실행 가능 plan 분리 작성:
    ...
    - [ ] [`W10-B active project decoupling`](superpowers/plans/2026-05-22-W10-B-active-project-decoupling.md) — `_active_project` 글로벌 제거, `SAA_ANALYSIS_FOLDER` → `SAA_ANALYSIS_ROOT` rename(legacy fallback), `acquire_scan_lock(scan_id)` asyncio.Lock per-scan mutex + `analysis_folder(root, pid, sid)` / `manifest_path(root, pid, sid)` helpers (D4/D5).
    - [ ] [`W10-C route surface`](superpowers/plans/2026-05-22-W10-C-route-surface.md) — projects CRUD service + schemas, `routes/projects.py` rewrite, `GET /projects/{pid}/scans`, 3 parallel sub-batches migrating routers to `/projects/{pid}/scans/{sid}/...`, acceptance gate (D6 force-create UX).
    - [ ] [`W10-D 프론트엔드`](superpowers/plans/2026-05-22-W10-D-frontend.md) — `state/projectSlice.ts` `activeScanId` 추가, `api/projects.ts` CRUD-only rewrite, `CreateProjectModal` + `ScanPicker` 컴포넌트, `App.tsx` 경로 grammar `/projects/:pid/scans/:sid/<tab>`, Sidebar rewrite + `CreateProjectForm` 삭제, 18 vitest 파일 3 parallel batches.
    - [ ] [`W10-E 테스트 + 데이터 마이그레이션`](superpowers/plans/2026-05-22-W10-E-test-and-data-migration.md) — `scripts/db/wipe-saa-test.sh` 가드(`saa_test` prefix 강제), `tests/api/conftest.py` rewire(`active_project`/`active_material`/`active_scan` fixtures), 27 backend test 파일 4 parallel batches, 신규 `test_w10_acceptance.py` (D2 DELETE-RESTRICT / D4 scan-mutex / D6 force-create), `scripts/dev/w10-acceptance.sh` 게이트.
