...
### 3.1 다음 한 발
**W10 진행 중(2026-05-23)** — W10-A 스키마 + W10-B active-project decoupling + **W10-C 라우트 서피스(완료)**. 다음 한 발: **W10-D** (프론트엔드 routes + ScanPicker + CreateProjectModal) → W10-E(테스트 sweep + 데이터 마이그레이션 + 잔여 stale 테스트 5건 처리) → backend restart → 1차 목표(scan 1건 GUI 업로드 e2e 검증). 그 뒤 잔여: W6.1 Cognito 부트스트랩(AWS 승인 대기) / W7(GPU) / W8 Part 2(SSM) / W9(reg_covar 캘리브레이션).
