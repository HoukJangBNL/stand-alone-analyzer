...
**Current branch**: `feat/migration-cutover` (post-cutover punch list 진행 중 — W3.5 / W2.4 / W8.1 / **W6 (auth+ACL+usage)** / **W5 전체(A/infra/B1/B2/C)** + **admin_usage 격리 fix** + **W10 A→E 전부** + **#64 fix** + **Robustness Phase A→D 전부** + **Phase E real-data 검증(E1-E5)** 완료. 다음 한 발: **W11 scan-guards 실행** (E5 gap 닫기 + new-scan-only 정책 codify) → 그 다음 **E6 3648/9GB 오너 승인**. 잔여: W6.1 Cognito 부트스트랩(AWS 승인 대기) / W7 GPU / W8.2 SSM / W9 reg_covar.)
...
### 3.1 다음 한 발
**Robustness Phase A→E 완료(2026-05-26)** — 프론트 visibility (A1-A4) / 서버 reliability (B1-B6) / Compute Tab 404 fix (C1) / draft·partial state semantics (D1-D5) / **real-data 검증 E1-E5**(scan_id 287..291, S3 ~151 objects). E1 single-file ✅ / E2 cancel-mid-flight ✅ / E3 retry-failed ✅ / E4 reopen-partial ✅(E2와 합산 검증) / E5 wrong-project guard 식별 → 별도 plan으로 분리(W11). CORS는 `localhost` 우회로 검증, `infra/s3/cors.json`에 `127.0.0.1` 추가 커밋(`1aa0014`)만 됨 — AWS apply는 오너 게이트. **다음 한 발**: **W11 scan-guards 실행** — [`plan`](superpowers/plans/2026-05-26-W11-scan-guards.md) — `presign_image_put`/`complete_image`/`finalize_scan`/`get_scan` + `list_scans`/`create_scan`이 caller↔project 멤버십을 검증하도록 가드 추가(`get_scan_for_user` / `require_editor_for_scan`), 외부 caller엔 `ScanNotFound` 404(존재 leak 방지) / 권한 부족엔 `Forbidden` 403. **UploadModal은 new-scan-only 정책 lock-in** — 드래프트 resume UX는 본 milestone 범위 외(상단 docstring으로 codify). 그 다음 **E6 3648 PNG / 9GB** 오너 승인 후. 잔여: W6.1 Cognito 부트스트랩(AWS 승인 대기) / W7(GPU) / W8 Part 2(SSM) / W9(reg_covar 캘리브레이션).
...
- [ ] **W11** scan access guards + new-scan-only 정책 codify — [`plan`](superpowers/plans/2026-05-26-W11-scan-guards.md) — Phase E5 gap 후속. `get_scan_for_user` / `require_editor_for_scan` 헬퍼 추가, 4개 scan-level 라우트 + `list_scans`/`create_scan`에 wire, outsider→404(leak 방지) / viewer→403. 별도 task로 `UploadModal` 상단에 new-scan-only 정책 docstring(자동 resume UX는 범위 외 명시). 9 task TDD plan, sub-skill: `subagent-driven-development`.
