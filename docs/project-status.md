...
**Current branch**: `feat/migration-cutover` (post-cutover punch list 진행 중 — W3.5 / W2.4 / W8.1 / **W6 (auth+ACL+usage)** / **W5 전체(A/infra/B1/B2/C)** + **admin_usage 격리 fix** + **W10 A→E 전부** 완료. 진행 중: #64 `test_scans_*` project_id 누락 수정(1차 목표 마지막 블로커). 그 다음: backend restart → 1차 목표(GUI 업로드 e2e). 잔여: W6.1 Cognito 부트스트랩(AWS 승인 대기) / W7 GPU / W8.2 SSM / W9 reg_covar.)
...
### 3.1 다음 한 발
**W10 A→E 전부 완료(2026-05-25)** — A 스키마 / B active-project decoupling / C 라우트 서피스 / D 프론트엔드 / E 테스트 sweep + acceptance specs + 게이트 스크립트. **현재 진행 중**: `test_scans_*` 13건 회귀 수정(#64) — W10-A 도입 후 `scans.project_id NOT NULL FK`인데 `upload_service.create_scan`이 `project_id`를 안 넘김. api-developer 위임 중. **다음**: #64 수정 → `bash scripts/dev/w10-acceptance.sh` 통과 → backend restart → 1차 목표(GUI 업로드 e2e). 그 뒤 잔여: W6.1 Cognito 부트스트랩(AWS 승인 대기) / W7(GPU) / W8 Part 2(SSM) / W9(reg_covar 캘리브레이션).
